...
export function FoundingPanel({ scenario, selectedRegionId, castleDraft, draftValid, locale, text, regionLocked = false, onSelectRegion, onConfirm }: FoundingPanelProps) {
  ...
  return (
    <section className={`founding-panel${selectedRegion ? ' placing-castle' : ' choosing-region'}`} aria-live="polite" aria-label={selectedRegion ? text.placeTitle : undefined} aria-labelledby={selectedRegion ? undefined : 'founding-title'}>
      {!selectedRegion ? (
        <>
          <header>
            <div><span className="founding-scenario-name">{scenario.name}</span><h1 id="founding-title">{text.chooseTitle}</h1><p>{text.chooseDescription}</p></div>
          </header>
        ...
        <div className="founding-actions">
          <div className="chosen-region"><i style={{ background: selectedRegion.color }}>{selectedRegion.index + 1}</i><span>{text.region} {selectedRegion.index + 1}</span></div>
